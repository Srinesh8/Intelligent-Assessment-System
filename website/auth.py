...
def calculate_test_percentages(test_results,subject):
    total_easy_questions = 0
    total_medium_questions = 0
    total_hard_questions = 0
    correct_easy_questions = 0
    correct_medium_questions = 0
    correct_hard_questions = 0

    for result in test_results:
        # print(result)
        if(result['subject']==subject):
            total_easy_questions += result['total_easy_questions']
            total_medium_questions += result['total_medium_questions']
            total_hard_questions += result['total_hard_questions']
            correct_easy_questions += result['correct_easy_questions']
            correct_medium_questions += result['correct_medium_questions']
            correct_hard_questions += result['correct_hard_questions']

    # Calculate percentages
    if total_easy_questions > 0:
        easy_percentage = (correct_easy_questions / total_easy_questions) * 100
    else:
        easy_percentage = 0.0

    if total_medium_questions > 0:
        medium_percentage = (correct_medium_questions / total_medium_questions) * 100
    else:
        medium_percentage = 0.0

    if total_hard_questions > 0:
        hard_percentage = (correct_hard_questions / total_hard_questions) * 100
    else:
        hard_percentage = 0.0

    # Calculate mean percentage
    total_tests = len(test_results)
    mean_percentage = (easy_percentage + medium_percentage + hard_percentage) / total_tests

    return {'easy_percentage':easy_percentage, 'medium_percentage':medium_percentage, 'hard_percentage':hard_percentage}


def get_questions_difficulty(num_questions,sub,difficulty):
    questions=[]
    if(sub=='java'):
...
def get_random_questions(num_questions, sub):
    ...
    percentages = mongo2.db.test_results.find_one({'email':email}) 

    if not existing_questions and not percentages:
        questions=get_questions(num_questions,sub)
        return questions
    if percentages:
        mean_percentages=[]
        mean_percentages=calculate_test_percentages(percentages['test_results'],sub)
        print(mean_percentages)
        # if(mean_percentages['easy_percentage']<60):
        #     questions=get_questions_difficulty(num_questions,sub,'easy')
        # elif(mean_percentages['medium_percentage']<50):
        #     questions=get_questions_difficulty(num_questions,sub,'medium')
        # elif(mean_percentages['hard_percentage']<40):
        #     questions=get_questions_difficulty(num_questions,sub,'hard')
        # else:
        #     questions=get_questions_difficulty(num_questions,sub,'hard')
    ...
    while len(questions) < num_questions:
        if(mean_percentages['easy_percentage']<60):
            random_question_cursor = get_questions_difficulty(num_questions,sub,'Easy')
            print(random_question_cursor)
        elif(mean_percentages['medium_percentage']<50):
            random_question_cursor=get_questions_difficulty(num_questions,sub,'Medium')
        elif(mean_percentages['hard_percentage']<40):
            random_question_cursor=get_questions_difficulty(num_questions,sub,'Hard')
        else:
            random_question_cursor=get_questions_difficulty(num_questions,sub,'Hard')
